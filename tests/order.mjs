...
await p.locator('.sizes button').nth(1).click(); await p.waitForTimeout(200); await snap('then picked 12 oz');
...
await p.locator('.sizes button').nth(2).click(); await p.waitForTimeout(200); await snap('switched to 16 oz');
...
await p.locator('.sizes button').nth(2).click(); await p.waitForTimeout(250); await snap('re-picked 16 oz (on quote)');
await b.close();
